...
# The Problem of Statistical Inference {.center background-color="black" background-image="images/kai-pilger-qHfJPxOnXi4-unsplash.jpg"}
...
text(8,5,"US Population", cex=2)
text(8,4,expression(rho=="?"), cex = 2)
```
...
text(2,5,"ANES Sample", cex=2)
text(2,4,expression(hat(p)==0.318))
arrows(6,4,4,4)
text(5,3.5,"Draw a sample", cex=1.3)
```
...
rect(6,1,10,9, col="skyblue")
text(8,5,"US Population", cex=2)
text(8,4,expression(rho%~~%0.318), cex = 2)
rect(0,3,4,7, col="yellow")
text(2,5,"ANES Sample", cex=2)
text(2,4,expression(hat(p)==0.318))
arrows(6,4,4,4)
text(5,3.5,"Draw a sample", cex=1.3)
arrows(4,6,6,6)
text(5,5.5,"Infer from sample", cex=1.3)
```
...
## When samples go bad `r emo::ji("imp")` {.smaller}
...
# The Sampling Distribution {.center background-color="black" background-image="images/tim-bennett-EoC_IuYmtug-unsplash.jpg"}
...
ggplot(students, aes(x=height))+
  ...
  labs(x="student height in inches", y="number of students",
       title="Population distribution of student height")
```
...
## How many samples of size 2 are possible? {.smaller}
...
```{r}
#| label: class-height-sampling-dist2
#| echo: false
#| fig-asp: 0.5625
...
```{r}
#| label: class-height-sampling-dist-compare
#| echo: false
#| fig-asp: 0.5625
...
ggplot(temp, aes(x=means, y=..density.., fill=size))+
  ...
  labs(x="mean height", fill = "sample size")+
  xlim(58, 76)+
  annotate("label", x=66.6, y=0.175, label="mu", parse=TRUE, size=5, color="red")+
...
## Its the Central Limit Theorem! {.smaller}
...
axis(1, at=c(-3,-1.96,-1,0,1,1.96,3), cex=2, las=3,
...
text(0,max(y)*0.9, expression(mu), col="red", cex=3, pos=4)
...
text(0, max(y)*0.5, label="68%", col="white", pos=3, cex = 2)
text(1.5, max(y)*0.3, label="95%", col="white", pos=3, cex = 2)
text(2.5, max(y)*0.05, label="99.7%", col="black", pos=3, cex = 2)
```
...
ggplot(data.frame(means5), aes(x=means5))+
  ...
  xlim(58, 76)+
  annotate("label", x=66.5, y=50000, label="most sample means\nwill be close to\nthe true value\nin the population", size = 7)+
  annotate("label", x=72, y=15000, label="but a few will\nbe far away", size = 7)+
  annotate("label", x=66.6, y=150000, label="mu", parse=TRUE, size=7, color="red")+
...
# Confidence Intervals {.center background-color="black" background-image="images/yogi-purnama-en7G3hTSjBQ-unsplash.jpg"}

## Consider this statement {.smaller}
...
text(0, max(y)/4, "95% probability that I will\ndraw a sample mean\nthat is within two (1.96)\nstandard deviations of\nthe true population mean", cex = 2)
```
...
## {{< fa calculator >}} Calculating the confidence interval {.smaller}
...
## {{< fa calculator >}} Calculating the t-statistic {.smaller}
...
## General form of the confidence interval {.smaller}
...
## Cheat sheet for SE and df {.smaller}
...
## Correlation coefficent example {.smaller}
...
# Hypothesis Tests {.center background-color="black" background-image="images/chuttersnap-UmncJq4KPcA-unsplash.jpg"}

## Game of make believe {.smaller}

:::: {.columns}

::: {.column width="30%"}

![](images/winnie_pooh.jpg)
...
## Lets set up a null hypothesis {.smaller}
### In English
...
## Coca-Cola Hypothesis Test {.smaller}
...
#### Assuming $H_0$ is true, what is the sampling distribution of my sample proportion? {.smaller}
...
## Coca-Cola Hypothesi Test {.smaller}
...
## The p-value is the endgame {.smaller}
...
## The critical value {.smaller}

:::: {.columns}

::: {.column width="30%"}

![](images/strongman-game.png)
...
## Hypothesis tests of relationships {.smaller}
...
## Example: Proportion differences {.smaller}
...
## `r emo::ji("thinking_face")` A p-value of 1.416426e-07? {.smaller}
...
## Example: Correlation coefficient {.smaller}
