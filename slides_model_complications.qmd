...
ggplot(augment(model), aes(x=.fitted, y=.resid))+
  ...
  labs(x="predicted values of box office returns by IMDB rating", y="model residuals")+
  theme_bw(base_size = 20)
```
...
- Residual plots can also help to detect **heteroskedasticity**.
...
text(40, 50, expression(y == 10 + 5*x + 0.04*x^2), cex = 1.5)
```
...
ggplot(augment(model), aes(x=.fitted, y=.std.resid))+
  ...
  geom_hline(yintercept=0, linetype=2)+
  geom_smooth(se = FALSE)+
  labs(x="fitted values of log box office returns by IMDB Rating",
       y="model residuals",
...
temp |>
  ...
  annotate("text", 1980, 0.015, size = 7,
...
```{r}
#| label: addhealthweight-dist-fig
#| echo: false
...
# Multicollinearity and Scale Creation {.center background-color="black" background-image="images/nareeta-martin-z7LRp54Yulw-unsplash.jpg"}

##  The problem of multicollinearity {.smaller}
...
```{r}
#| label: structural-multicol-compare
...
ha_shp <- st_read(here(here("data","nyc_health_areas","health_area_shapefiles",
                            "nyha.shp")), quiet = TRUE)
ha_shp <- ha_shp |>
  mutate(health_area = BoroCode*10000+HealthArea) |>
  right_join(nyc) |>
  mutate(amtcapita_d=cut(amtcapita, 
                         breaks=quantile(amtcapita, seq(from=0, to=1, by=0.2)),
                         include.lowest=TRUE))

ggplot(ha_shp, aes(fill=amtcapita_d))+
  geom_sf()+
...
```{r}
#| label: tbl-model-nyc-multicol
#| tbl-cap: Linear models predicting per-capita social service spending (logged)
#| echo: false

cm <- c("(Intercept)" = "intercept",
        "poverty" = "poverty rate",
        "unemployed" = "unemployment rate",
        "lincome" = "median income (logged)")

nyc$lincome <- log(nyc$income)
model.pov <- lm(log(amtcapita)~poverty, data=nyc)
model.unemp <- lm(log(amtcapita)~unemployed, data=nyc)
model.income <- lm(log(amtcapita)~lincome, data=nyc)
...
##  The correlation matrix as diagnostic {.smaller}
...
##  You have multicollinearity, now what? {.smaller}
...
##  Standardization and reverse ordering {.smaller}
...
```{r}
#| label: standardize-reverse-code
...
::: {.callout-warning .fragment .fade-up}
...
## Estimation in factor analysis {.smaller}
...
```{r}
#| label: factor-loading-fig

fa.diagram(factor_nyc, cex = 2f)
```
...
```{r}
#| label: correlogram-moral-fig
#| fig-asp: 0.5625

corrgram(morality_r, upper.panel=panel.cor, order=TRUE, cex = 1.25)
```
...
## Comparing different number of factors {.smaller}
...
fa.diagram(morality_fa1, main = NULL, cex = 5)
fa.diagram(morality_fa2, main = NULL, cex = 5)
fa.diagram(morality_fa3, main = NULL, cex = 5)
```
...
# Model Selection {.center background-color="black" background-image="images/caleb-jones-J3JMyXWQHXU-unsplash.jpg"}

##  Predicting violent crime rates

Lets predict violent crime rates in US States by a variety of characteristics. 

```{r}
#| label: build-crime-models

model_demog <- lm(violent_rate ~ median_age + percent_male, data = crimes)
model_ineq <- update(model_demog, .~. + gini)
model_pov <- update(model_ineq, .~. + poverty_rate)
model_unemp <- update(model_pov, .~. + unemploy_rate)
model_justunemp <- lm(violent_rate ~ unemploy_rate, data = crimes)
model_another <- update(model_justunemp,.~. + gini + median_age)
```

Note that some of these models are **nested** within other models, meaning that the more complex models just add additional variables to earlier models. Other models are non-nested. 
...
modelsummary(list(model_demog, model_ineq, model_pov, model_unemp, 
                  model_justunemp, model_another),
             output = "gt",
             fmt = 1,
             statistic = NULL,
             coef_map = cm,
...
##  A tradeoff: accuracy vs. parsimony {.smaller}
...
### Accuracy
 We want our models to be accurate, or more formally we want them to have high **goodness of fit**. This is measured in linear models primarily by the $R^2$ value. A more accurate model accounts for more of the variation in the dependent variable. 
...
- **Do not** choose a model based on all of the coefficients being statistically significant.
- **Do not** choose a model because it produces the results you like the best. That is not science. 
- **Do not** choose a model based on some purely mechanical algorithm. Model choice should be driven by what questions you want to ask as much as goodness of fit and parsimony.
- **Do not** choose a model based on $R^2$ values. This approach will ignore parsimony.

:::

::: {.column .fragment}

### {{< fa check >}} Good Practices
...
##  {{< fa toolbox >}} Model selection tools
...
All three of these statistics (and more) are provided in `glance` from the `broom` package.
...
##  Null vs. saturated model {.smaller}
...
The null model is the linear model with no predictors.
...
Where,
$$n=p$$
The saturated model is a hypothetical model in which the number of predictors equals the number of observations and we fit each $y_i$ exactly.

:::

##  The F-Test {.smaller}

- Compare two different nested models, where the second model has $g$ additional independent variables.
- The null hypothesis for the F-test is that the effects for all of the additional $g$ variables are zero. 
...
-  $SSR$ is the sum of squared residuals for a given model
- $g$ is the additional terms added to the second model
- $k$ is the number of terms in the first model

::: {.fragment}

Assuming the null hypothesis is true, the F-statistic will come from an F-distribution with degrees of freedom equal to $g$ and $n-g-k-1$ (the F-distribution has two DF parameters). The area in the right tail gives the p-value.
...
The p-value for the hypothesis test is 0.0011. So, I would reject the null hypothesis and prefer the model predicting violent crime by median age and percent male to the null model. 
...
tibble(x, y) |>
  ggplot(aes(x = x, y = y))+
  geom_line()+
  geom_vline(xintercept = fstat, linetype = 2)+
  labs(x = "F-statistic", y = NULL,
       title = "F-distribution (2, 47)")
```
:::
...
```{r}
#| label: ftest-anova-nested

anova(model_null, model_demog, model_ineq, model_unemp)
```

##  F-test for adding one variable

```{r}
#| label: ftest-anova-onecar

anova(model_demog, model_ineq)
summary(model_ineq)$coef["gini",]
```

- The p-value for an F-test where only one variable is added is identical to the p-value for a t-statistic of that particular regression coefficient.
...
```{r}
#| label: adj-r-squared-comparison

glance(model_demog)[,c("r.squared", "adj.r.squared")]
```
...
The equation for BIC' for linear models is most intuitive:
...
bic.null(model_demog)
BIC(model_demog)
BIC(model_demog)-BIC(model_null)
```
...
bic.null(model_unemp) - bic.null(model_demog)
BIC(model_unemp) - BIC(model_demog)
```
...
```{r}
#| label: tbl-model-summary-compare
#| tbl-cap: Models predicting violent crime rates
#| echo: false
...
library(BMA)
model_bma <- bic.glm(violent_rate ~ median_age + percent_male + percent_lhs + 
                       median_income + gini + unemploy_rate + poverty_rate, 
        data = crimes, glm.family = gaussian)
summary(model_bma)
```

```
 28  models were selected
 Best  5  models (cumulative posterior probability =  0.4647 ): 

               p!=0   EV         SD         model 1     model 2     model 3     model 4     model 5   
Intercept      100    82.937285  1.409e+03  1020.97063  -849.27477  -191.04334  1116.67098  1040.47540
median_age     68.4  -17.300522  1.453e+01   -28.22382       .       -13.77369   -30.04257   -28.12604
percent_male   14.9    1.752056  1.613e+01       .           .           .           .           .    
percent_lhs    61.5  -16.396241  1.647e+01       .       -29.60906   -28.01411   -13.21131   -28.89025
median_income  56.2    0.005413  5.728e-03       .         0.01155     0.01018       .           .    
gini           16.3    1.577432  8.166e+00       .           .           .           .           .    
unemploy_rate  58.2   43.668082  4.307e+01    79.46096       .           .       100.40607    85.81370
poverty_rate   63.8   42.509637  4.017e+01       .        88.73749    81.90378       .        26.37693
                                                                                                      
nVar                                             2           3           4           3           4    
BIC                                         -135.92479  -135.70055  -134.70002  -134.09916  -133.99923
post prob                                      0.144       0.129       0.078       0.058       0.055  
```
