...
ggplot(president, aes(x=gender, y=percent))+
  ...
  labs(x=NULL, y="Percent of voters voting for Trump",
       title="Confidence intervals on a proportion",
       caption="ANES data, 2016")
```
:::
::::
...
ggplot(crimes, aes(x=poverty_rate/100, y=property_rate))+
  ...
  ylim(0,5000)+
  annotate(label = "Run", x=0.18, y = df[1]*0.95, size = 4, geom = "text")+
  annotate(label = "Slope", x=0.185, y = df[1]*1.07, size= 4, geom = "text")+
  annotate(label = "Rise", x=0.2*1.01, y = df[1]+(df[2]-df[1])/2, hjust = 0, 
           size = 4, geom = "text")+
  annotate(label = "Y-intercept", x = 0, y = coef(model)[1]*0.91,  hjust = 0, 
           geom = "text", size = 4)+
  annotate(geom = "text",
